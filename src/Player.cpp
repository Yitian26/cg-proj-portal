...
void Player::processInput(const InputManager &input, Scene *scene, float dt) {
    ...
    auto hit = scene->physicsSystem->raycast(camera.Position, camera.Front, 5.0f);
    if (hit.hit && hit.object) {
        if (auto door = dynamic_cast<Door*>(hit.object)) {
            door->toggle(position);
            }
        }
    }
    ...
    if (input.isMousePressed(GLFW_MOUSE_BUTTON_LEFT)) {
        if (isGrabbing) {
            ...
            scene->portalA->checkRaycast(result,camera.Right,camera.Front);
        }
    }
    if (input.isMousePressed(GLFW_MOUSE_BUTTON_RIGHT) && !isGrabbing) {
        scene->portalGun->fire();
        auto result = scene->physicsSystem->raycast(camera.Position, camera.Front, 100.0f);
        scene->portalB->checkRaycast(result,camera.Right,camera.Front);
    }
