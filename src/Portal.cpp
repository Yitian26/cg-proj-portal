...
void Portal::checkRaycast(RaycastHit result, glm::vec3 playerRight,glm::vec3 rayDir) {
    if (result.hit) {
        isActive = true;
        ...
        if (glm::dot(result.normal, rayDir) > 0.0f) {
...
void Portal::reset() {
    this->isActive = false;
    this->position = glm::vec3(0.0f, -2000.0f, 0.0f);
    updateFramesTransform();
    glm::vec3 axes[3] = { glm::vec3(1,0,0), glm::vec3(0,1,0), glm::vec3(0,0,1) };
    
    if (nearTrigger) {
        nearTrigger->setFromCenterAxesExtents(this->position, axes, glm::vec3(0.0f));
    }
    if (teleportTrigger) {
        teleportTrigger->setFromCenterAxesExtents(this->position, axes, glm::vec3(0.0f));
    }
}